...
void Histo::addToBin(float val) {
	int intVal;
	if (val < _minData) _numBelow++; 
	else if (val > _maxData) _numAbove++;
	else {
		if (_maxData == _minData) intVal = 0;
		else intVal = (int)(((double) val - (double) _minData)*(double)_numBins/((double) _maxData - (double) _minData));
